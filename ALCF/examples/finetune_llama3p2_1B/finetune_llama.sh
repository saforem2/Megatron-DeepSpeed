DS_CONFIG=./ALCF/examples/finetune_llama3p2_1B/ds_config.json
DS_CONFIG_EMPTY=./ALCF/examples/finetune_llama3p2_1B/ds_config_empty.json
DATASET_PATH=./ALCF/examples/finetune_llama3p2_1B/alpaca_data.json
...
NUM_KV_HEADS=$(cat "${HF_CONFIG}" | jq -r '.num_attention_heads')
SEQ_LENGTH=512
######################################

MEGA_DS_LLAMA_PATH=./"llama-3p2-1B-mega-ds-T${TP}P${PP}"
...
if [ "$1" = "convert_hf2mds" ]; then
    DS_CONFIG_PATH="./ALCF/examples/finetune_llama3p2_1B/ds_config_empty.json"
elif [ "$1" = "convert_mds2hf" ]; then
    DS_CONFIG_PATH="./ALCF/examples/finetune_llama3p2_1B/ds_config_empty.json"
else
    DS_CONFIG_PATH="./ALCF/examples/finetune_llama3p2_1B/ds_config.json"
fi
...
printf "task_args: %s\n" $task_args[@] | tee "${OUTFILE}"
printf "comm_args: %s\n" $comm_args[@] | tee -a "${OUTFILE}"
