    ...
    def forward(self, q, k, v):
        ...
        if self.training:
            ...
            cu_seqlens_k = (
                ...
                if get_accelerator().device_name() == 'cuda' else None
            )
            dropout_p = 0

        assert self.flash_attn_func is not None
        if self.use_flash_attn:
            ...
            #################################################################
            # BUG: When trying to `--use-flash-attn-builder` on XPU
            # - Trying to call:
            #
            #   ```python
            #   >>> output = self.flash_attn_func(
            ...
            #   ```
            #
            # - This (internally) then tries to call
            # `torch.nn.functional.scaled_dot_product_attention`,
            # which crashes with the `RuntimeError` shown below:
            #
            #   ```python
            ...
            #   ```python
            #   >>> print(f"{q.shape=}\n{k.shape=}\n{v.shape=}")
            #   >>> q.shape=torch.Size([2, 16, 4096, 128])
            #   >>> k.shape=torch.Size([2, 64, 4096, 128])
            #   >>> v.shape=torch.Size([2, 64, 4096, 128])
            #   ```
            #
            ######################################################
