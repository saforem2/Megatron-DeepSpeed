...
if __name__ == "__main__":
    ...
    inv_freq = 1.0 / (
        mds_model["args"].__dict__["rope_theta"]
        ** (torch.arange(0, dim, 2).float() / dim)
    )
    hidden_size = mds_model["args"].__dict__["hidden_size"]
    kv_dim = (
        mds_model["args"].__dict__["kv_channels"]
        * mds_model["args"].__dict__["num_key_value_heads"]
    )
    kv_groups = (
        mds_model["args"].__dict__["num_attention_heads"]
        // mds_model["args"].__dict__["num_key_value_heads"]
    )
    for layer_i in range(Updated_Llama_config.__dict__["num_hidden_layers"]):
        ...
        fused_qkv = mds_model["module"]["language_model"]["encoder"][
            f"layers.{layer_i}.self_attention.query_key_value.weight"
        ]
        state_dict[f"model.layers.{layer_i}.self_attn.q_proj.weight"] = fused_qkv[
            0:hidden_size
        ]
        state_dict[f"model.layers.{layer_i}.self_attn.k_proj.weight"] = fused_qkv[
            hidden_size : hidden_size + kv_dim
        ]
        ...
        state_dict[f"model.layers.{layer_i}.self_attn.v_proj.weight"] = fused_qkv[
            hidden_size + kv_dim : hidden_size + 2 * kv_dim
        ]
        ...
        state_dict[f"model.layers.{layer_i}.self_attn.o_proj.weight"] = mds_model[
            "module"
        ]["language_model"]["encoder"][f"layers.{layer_i}.self_attention.dense.weight"]
        # MLP Layers
        fused_mlp = mds_model["module"]["language_model"]["encoder"][
            f"layers.{layer_i}.mlp.dense_h_to_4h.weight"
        ]
        ...
        state_dict[f"model.layers.{layer_i}.mlp.down_proj.weight"] = mds_model[
            "module"
        ]["language_model"]["encoder"][f"layers.{layer_i}.mlp.dense_4h_to_h.weight"]
        # LayerNorm weights and RoPe
        state_dict[f"model.layers.{layer_i}.input_layernorm.weight"] = mds_model[
            "module"
        ]["language_model"]["encoder"][f"layers.{layer_i}.input_layernorm.weight"]
        state_dict[
            f"model.layers.{layer_i}.post_attention_layernorm.weight"
        ] = mds_model["module"]["language_model"]["encoder"][
    ...
    state_dict["model.embed_tokens.weight"] = mds_model["module"]["language_model"][
        "embedding"
    ]["word_embeddings"]["weight"]
    state_dict["model.norm.weight"] = mds_model["module"]["language_model"]["encoder"][
        "final_layernorm.weight"
    ]
    state_dict["lm_head.weight"] = mds_model["module"]["language_model"][
        "output_layer"
    ]["weight"]
